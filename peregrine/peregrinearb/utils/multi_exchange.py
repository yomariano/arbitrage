    ...
    if fees:
        for exchange_dict in exchanges:
            if 'maker' in exchange_dict['object'].fees['trading']:
                ...
                warnings.warn("The fees for {} is {}.".format(exchange_dict['object'].id, exchange_dict['fee']))
            else:
...
async def _add_exchange_to_multi_digraph_one_by_one(graph: nx.MultiDiGraph, exchange, log=True, suppress=None):
    tasks = [_add_market_to_multi_digraph(exchange, ticker, graph, log=log, suppress=suppress)
             for ticker in await exchange['object'].fetch_ticker]
...
async def _add_exchange_to_multi_digraph(graph: nx.MultiDiGraph, exchange, log=True, suppress=None):
    if exchange['object'].has['fetchTickers']:
        tickers = await exchange['object'].fetch_tickers()
        tasks = [_add_market_to_multi_digraph(exchange, market_name, tickers[market_name], graph, log=log, suppress=suppress)
                for market_name in tickers]
    else:
        tasks = [_add_market_to_multi_digraph(exchange, market_name, await exchange['object'].fetch_ticker(market_name), graph, log=log, suppress=suppress)
             for market_name in exchange['object'].symbols]
...
# ccxt's fetch_ticker
async def _add_market_to_multi_digraph(exchange, market_name, ticker, graph: nx.DiGraph, log=True, suppress=None):
    if suppress is None:
